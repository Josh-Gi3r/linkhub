/**
 * SDK — session management + (optional) legacy OAuth integration.
 *
 * The magic-link auth flow in auth.ts only uses createSessionToken / verifySession /
 * authenticateRequest.  The OAuthService / SDKServer methods are kept for
 * back-compat but are not called by the default magic-link setup.
 *
 * SECURITY: getSessionSecret() reads ENV.cookieSecret which throws if JWT_SECRET
 * is unset, preventing tokens from being signed with an empty key.
 */
...
class OAuthService {
  constructor(private client: ReturnType<typeof axios.create>) {
    if (!ENV.oAuthServerUrl) {
      console.warn(
        "[OAuth] OAUTH_SERVER_URL is not configured. " +
          "Legacy OAuth exchange will fail; magic-link auth is unaffected."
      );
    }
  }
...
class SDKServer {
  ...
  private getSessionSecret(): Uint8Array {
    // ENV.cookieSecret getter throws if JWT_SECRET is empty — intentional fail-closed
  ...
  async authenticateRequest(req: Request): Promise<User> {
    ...
    // If user not in DB, attempt sync from OAuth server (legacy flow)
    if (!user) {
      try {
